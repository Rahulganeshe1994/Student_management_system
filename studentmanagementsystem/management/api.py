from management .models import Teacher , User , Courses , Student
...
class UserModelView1(APIView):
    def put(self,request,pk):
        try:
            ...
            serializer = UserSerializer(instance = data , data=request.data , partical = True)
...
class CourseModelView1(APIView):
    def put(self,request,pk):
        try:
            ...
            serializer = CoursesSerializer(instance=data , data = request,partial=True)
            if serializer.is_valid():
                serializer.save()
            else:
                return Response(status=status.HTTP_200_OK)
